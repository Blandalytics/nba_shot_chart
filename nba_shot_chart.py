...
with col1:
    ...
    st.markdown("NBA players earn points through a combination of taking shots (and free throws), taking quality shots, and making those shots (and free throws). This app is designed to illustrate *how* a player scored (or didn't score) points in a given game, and visually represent the quality of their shot selection. :green[Green] means the player made the shot and/or added points in that category. :violet[Purple] means the opposite.")
    st.write("The [xFG% model](https://github.com/Blandalytics/nba_shot_chart/blob/main/xFG.png?raw=true) was trained on location data from the 2022-23, 2023-24, and 2024-25 seasons. If you're interested in the xFG% value of each X,Y, coordinate, a csv can be found [here](https://github.com/Blandalytics/nba_shot_chart/blob/main/nba_xFG_values.csv).")
    st.write("The xFTA model was trained on 2022-23, 2023-24, and 2024-25 data. The target variable was per-game FTA/shot, using the per-game distribution of shots in the following areas: [Restricted Area, Paint, Mid-Range, Corner 3, and Above-the-Break 3](https://github.com/Blandalytics/nba_shot_chart/blob/main/shot_loc_classes.png).")
