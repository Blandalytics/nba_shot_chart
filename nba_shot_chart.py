...
def shot_summary(player_id,game_date=game_date, season_long=season_long):
    ...
    if abs(actual_points-expected_points) >= 0.025:
        cb.ax.annotate("", xytext=(expected_points+0.0025, 0.5), xy=(actual_points+0.003, 0.5),
                       arrowprops=dict(color='w',
                                       arrowstyle="-|>",
                                       edgecolor='w',
                                       shrinkA=2,shrinkB=0,
                                       linewidth=4.5))
        cb.ax.annotate("", xytext=(expected_points+0.0025, 0.5), xy=(actual_points+0.0025, 0.5),
